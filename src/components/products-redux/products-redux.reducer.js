...
const reducerName = 'products-redux';
...
export function exampleAsyncAction() {
    return (dispatch, getState) => {
        const { productsReduxReducer } = getState();
        dispatch(exampleAction(!productsReduxReducer.exampleVariable));
    };
}
...
export default function productsReduxReducer(state = initialSate, action) {
    switch (action.type) {
